...
class Grade(FlaskForm):
    ...
    term = RadioField('Wybierz semestr', choices=[('term1', 'Semestr1'), ('term2', 'Semestr2')])
...
def addGrade():
    ...
    if gradeForm.validate_on_submit():
        ...
        with open('data/grades.json', 'w', encoding='utf-8') as gradesFile:
            json.dump(grades, gradesFile)
            gradesFile.close()
    return render_template('add-grade.html', title='Dodaj ocenę', userLogin=session.get('userLogin'),
                           firstName=session.get('firstName'), date=date, gradeForm=gradeForm)
